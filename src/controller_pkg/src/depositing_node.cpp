...
private:
    ...
    void execute(const std::shared_ptr<GoalHandleDepositing> goal_handle)
    {
        ...
        send_feedback(goal_handle, "Depositing started");

        send_feedback(goal_handle, "Stage 1: Raising bucket and tilting");
        if (!MoveBucket(3.2f, 2.0f, false, goal_handle))
        { result->success = false; goal_handle->canceled(result); return; }
        send_feedback(goal_handle, "Stage 1 complete");

        send_feedback(goal_handle, "Stage 2: Vibrating material out (10s)");
        ...
        send_feedback(goal_handle, "Stage 3: Resetting bucket to home");
        ...
        send_feedback(goal_handle, "Depositing complete — bucket reset");
        result->success = true;
