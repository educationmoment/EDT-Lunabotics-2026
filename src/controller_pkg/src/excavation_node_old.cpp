...
private:
    ...
    void execute(const std::shared_ptr<GoalHandleExcavation> goal_handle)
    {
        ...
        if (!MoveBucket(-0.4f, -0.93f + buffer_, false, 0.0f, goal_handle))
        ...
        MoveBucket(-0.43f, -0.93f + buffer_, true, 1500.0f, goal_handle);
        auto dig_timer1 = std::chrono::high_resolution_clock::now();
        while (std::chrono::duration_cast<std::chrono::seconds>(
                   std::chrono::high_resolution_clock::now() - dig_timer1).count() < 1)
        {
            ...
            MoveBucket(-0.37f, -0.85f + buffer_, true, 1500.0f, goal_handle);
            std::this_thread::sleep_for(std::chrono::milliseconds(5));
        }
        ...
        while (std::chrono::duration_cast<std::chrono::seconds>(
                   std::chrono::high_resolution_clock::now() - dig_timer2).count() < 1)
        {
            if (goal_handle->is_canceling()) { result->success = false; goal_handle->canceled(result); return; }
            leftDrive.SetVelocity(1000.0f); rightDrive.SetVelocity(1000.0f);
            vibrator.SetDutyCycle(VIBRATOR_DUTY);
            MoveBucket(-0.37f, -0.85f + buffer_, true, 1000.0f, goal_handle);
            std::this_thread::sleep_for(std::chrono::milliseconds(5));
        }
        ...
        while (std::chrono::duration_cast<std::chrono::seconds>(
                   std::chrono::high_resolution_clock::now() - dig_timer3).count() < 1)
        {
            if (goal_handle->is_canceling()) { result->success = false; goal_handle->canceled(result); return; }
            leftDrive.SetVelocity(1000.0f); rightDrive.SetVelocity(1000.0f);
            vibrator.SetDutyCycle(VIBRATOR_DUTY);
            MoveBucket(-0.4f, -0.88f + buffer_, true, 1000.0f, goal_handle);
            std::this_thread::sleep_for(std::chrono::milliseconds(5));
        }

        send_feedback(goal_handle, "Stage 4 complete — stopping drivetrain");
