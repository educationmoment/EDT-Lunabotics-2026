...
/**
 * @brief Callback for Excavation Controll. On invocation, will start an excavation cycle
 *        assuming request->start_excavation is TRUE. It creates SparkMax objects to communicate
 *        and then proceeds through the excavation action.
 * @param request Request contains the start_excavation field which should be true to run the service.
 * @param response Response contains the excavation_successful field.
 *********************************************************************/
         ...
         std::shared_ptr<interfaces_pkg::srv::ExcavationRequest::Response> response)
{
    ...
    // Lift Bucket Position to 1.75f, if 10 seconds elapse and condition not met,
    // timeout the action. Set tilt actuator to 0.0.
