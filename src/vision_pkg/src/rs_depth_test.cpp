...
public:
  MultiCameraNode() : Node("multi_camera_node") {
    ...
    obstacle_detection_pub_ = this->create_publisher<std_msgs::msg::Bool>("rs_node/camera1/obstacle_detection", 10);
...
private:
  ...
  rclcpp::Publisher<std_msgs::msg::Bool>::SharedPtr obstacle_detection_pub_;
  ...
  void timer_callback() {
    ...
    if (pipelines_.size() >= 1) {
      ...
      if (pipelines_[0].poll_for_frames(&frames1)) {
        ...
        if (depth_frame1 && depth_frame1.get_data()) obstacle_detection_callback(depth_frame1, obstacle_detection_pub_);
      } else {
  ...
  void obstacle_detection_callback(const rs2::frame& depth_frame, rclcpp::Publisher<std_msgs::msg::Bool>::SharedPtr pub){
    const int GROUND_DEPTH_MM = 800;
    const int POS_THRESHOLD = 500; //Half the size of a rick
    const int NEG_THRESHOLD = 500; //Half the size of a crator
    ...
    for (int y = height / 2; y < height; y += 5) {
      for (int x = 0; x < width; x += 5) {
        ...
        if (delta > POS_THRESHOLD) pos_count++;
        else if (-delta > NEG_THRESHOLD) neg_count++;
      }
    }
