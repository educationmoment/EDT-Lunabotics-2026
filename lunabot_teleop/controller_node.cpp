...
#include "interfaces_pkg/srv/navigation_request.hpp"
#include <cmath>
#include <string>
#include <cstdlib>
#include <algorithm>
...
public:
        ...
        prev_alternate_button_(false)
  {
    ...
    leftLift.SetP(0, 1.5099999904632568f); 
    leftLift.SetI(0, 0.0f);
    leftLift.SetD(0, 0.0f);
    leftLift.SetF(0, 0.0002099999983329326f);
    // PID settings for left lift

    rightLift.SetP(0, 1.5099999904632568f);
    rightLift.SetI(0, 0.0f);
    rightLift.SetD(0, 0.0f);
    rightLift.SetF(0, 0.0002099999983329326f);
    ...
    RCLCPP_INFO(this->get_logger(), "Initializing navigation client");
    navigation_client_ = (this->create_client<interfaces_pkg::srv::NavigationRequest>("navigation_service"));
...
private:
  ...
  rclcpp::Client<interfaces_pkg::srv::NavigationRequest>::SharedPtr navigation_client_;
...
/**
 * @brief Sends request to navigation node and manages response
 * @param None
 * @returns None
 */
    void send_navigation_request()
    {
        if (!navigation_client_ || !navigation_client_->wait_for_service(std::chrono::seconds(1)))
        {
            RCLCPP_ERROR(this->get_logger(), "Navigation service not available");
            return;
        }
        auto request = std::make_shared<interfaces_pkg::srv::NavigationRequest::Request>();
        request->start_navigation = true;
        request->target_x = 4.0;  // Modify these as needed
        request->target_y = 1.0;
        request->orientation_z = 0.707;
        request->orientation_w = 0.707;
    
        navigation_client_->async_send_request(request, 
            [this](rclcpp::Client<interfaces_pkg::srv::NavigationRequest>::SharedFuture future)
            {
                auto response = future.get();
                if (response->navigation_successful) {
    ...
    // NAVIGATION AUTONOMY (Window button - button 8)
    // You can change this to any button you prefer
    bool current_nav_button = (joy_msg->buttons[Gp::Buttons::_WINDOW_KEY] > 0);
    static bool prev_nav_button = false;
    if (current_nav_button && !prev_nav_button)
    {
        send_navigation_request();
    }
    prev_nav_button = current_nav_button;
